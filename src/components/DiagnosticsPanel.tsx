...
export default function DiagnosticsPanel({ open, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/95 text-neutral-100 flex flex-col">
      ...
      {/* Demo Mode */}
      <div className="border-b border-neutral-800 px-4 py-3 bg-neutral-950">
        <div className="flex items-center justify-between mb-2">
          <div>
            <div className="text-xs uppercase tracking-widest text-neutral-500">Data Source</div>
        ...
        <div className="flex gap-2 flex-wrap">
          <button
            onClick={setLive}
            className={`px-3 py-2 rounded font-bold text-sm transition-colors ${!inDemo ? 'bg-feasible text-white' : 'bg-neutral-800 text-neutral-400 hover:text-feasible border border-neutral-700'}`}
          >
            ● Live
          </button>
          {PHASES.map((p) => {
            ...
            return (
              <button
                key={p}
                onClick={() => setDemo(p)}
                className={`px-3 py-2 rounded font-bold text-sm transition-colors ${active ? 'bg-gold text-neutral-900' : 'bg-neutral-800 text-neutral-400 hover:text-gold border border-neutral-700'}`}
