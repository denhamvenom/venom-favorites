...
- **Demo mode (post-deploy add)** ✓ — Diagnostics panel (5-tap on version footer) now exposes a Data Source toggle: **Live** (live 2026 API) and **Demo A/B/C/D/E** (2025 Championship replay at 5 progressive cutoffs). Phase B specifically shows roughly half the quals played with scores + the rest still upcoming on the schedule (cutoff: Thursday 3 PM CDT). Toggling is persisted via `localStorage.dataSourceOverride` and `localStorage.fixturePhase`; tapping a button reloads the page so all polling/state resets cleanly. Fixtures are lazy-bundled (Vite emits separate chunks); the SW `globIgnores` excludes them from precache so initial load is unchanged. Cutoffs: A=pre-event · B=Thu 3PM · C=Fri 8PM · D=Sat 7:30AM · E=Sat end.
- **Per-field cycle time** ✓ — Renamed `DEFAULT_MATCH_DURATION_MIN` (was 7) → `DEFAULT_CYCLE_TIME_MIN = 8` to match 2026's start-to-start cadence. Added `src/logic/cycle.ts` with `computeFieldCycle` (median of consecutive `actualStart` deltas over last 5 completed) + `effectiveCycleMin` resolver: observed value wins when `basedOn ≥ 2`, otherwise per-year baseline. `useSchedule` exposes `cycles: FieldCycle[]` alongside drifts and emits a `[schedule]` debug log per field per refresh ("ARCHIMEDES cycle: 8.2 min"). `planSchedule` + `summarize` accept `cycles` + `defaultCycleTimeMin` options. The walk-time editor now has a small "Cycle time" input above the matrix (persisted in `cycle-override/v1`) and shows observed cycle chips per field. Reset button clears both. `median()` extracted into `src/logic/stats.ts` and shared between drift/cycle. 11 new cycle unit tests + 3 cycle-aware planner tests; **101 tests total**.
- **Day 1 QA + watching anchor + SW reload (2026-04-30, mid-event late afternoon)** ✓ — Six fixes shipped together after user reported on-event bugs with live 2026 data:
  1. **Hide "Qualifying" badge** — removed `qualifying:` from `STATUS_BADGES` map in `FavoritesList.tsx`. During regular qual play, favorites show team#/division only — no pill. Other states (selected/eliminated/division_winner/awaiting_selection/not_selected) still render badges.
  2. **buildMatches: schedule ∪ matches join (root cause for the bogus "Awaiting Selection" + "only 1 of 3 matches in Results" symptoms)** — Previously `buildMatches` iterated only the schedule array, populating alliance arrays from `sched.teams`. On Day 1 the FRC API returns matches before the schedule fully publishes them (or with empty `teams`), so deriveProgress filtered out played matches with empty alliance arrays. New algorithm iterates the union of (schedule keys ∪ match keys), prefers `live.teams` over `sched.teams` when populated, and falls back gracefully. New transform.test.ts has 5 cases covering matches-only, schedule-only, divergent rosters, and the join. Side effect: `deriveStatus`'s `teamQuals` filter now finds all of a favorite's quals → `remaining.length > 0` on Day 1 → returns `'qualifying'` (which now renders no badge).
  3. **TopBar "Next Up" — skip played + tighten window** — Added `m.redScore === undefined || m.blueScore === undefined` filter and tightened past-window from 5 min to 8 min. A just-finished match no longer surfaces as "Next Up."
  4. **"I'm watching this match" anchor** — New `src/state/watching.ts` (`useWatching` hook) tracks the user's active match (`watching-match/v1`) and decays to a memory of last-known location (`last-location/v1`, 30-min TTL) when scores arrive. UI: "I'm here" button on each MatchCard (qual/playoff, not played); active card gets purple ring + "I'M WATCHING" pill; TopBar swaps to "Watching now: ARC" mode. Planner integration: `planSchedule` accepts an optional `anchor: { field, busyUntil?, label? }` and synthesises a virtual previous-match for first-real-match walk feasibility. Active mode's `busyUntil` flags conflicts as "Starts before Q35 ends — you can't make it." Memory mode (no `busyUntil`) walks from the field with no time constraint. Auto-transitions on score arrival so nothing stays in "watching" past its life. 4 conflicts tests + 5 watching tests.
  5. **TBA verification (diagnostic-only)** — New `src/api/tba.ts` client + division→event-key map. Diagnostics panel adds a "Verify with TBA" button that fetches `/event/{eventKey}/matches/simple` for each favorite-division and surfaces a per-division mismatch count (presence-in-FRC vs presence-in-TBA + alliance-roster diff). Read-only, lazy (only on button press). `VITE_TBA_API_KEY` lives in `.env.local`. Graceful fallback message when the env var is missing.
  6. **SW reload banner** — New `src/state/swUpdate.ts` wraps `virtual:pwa-register` and exposes `{ needRefresh, reload }`. App-level effect auto-reloads 3 seconds after detecting `onNeedRefresh`. Toast renders bottom-center: "New version available — reloading…" Pairs with the `registerType: 'autoUpdate'` already in vite.config.ts.

  117 tests; live verification ✓ via chrome-devtools-mcp against https://venom-favorites.web.app with 8044 → HOPPER on Day 1 quals. New env: `VITE_TBA_API_KEY` (in `.env.local`). New localStorage keys: `watching-match/v1`, `last-location/v1`. No existing storage keys touched.

  **Live-test follow-up (commit 77431e1):** initial deploy still showed 'awaiting_selection' for 8044 + only Q115 visible in the schedule. Root cause via live API capture: FRC API returns `scoreRedFinal: null` (not `undefined`) for unplayed matches. `m.redScore === undefined` checks throughout the codebase silently misclassified all of 8044's 10 quals as played (3 actually played, 7 still upcoming). Fixed at the boundary in `buildMatches` with `live?.scoreRedFinal ?? undefined`. After redeploy: status correctly empty (qualifying), all 7 upcoming matches visible in correct chronological order, Q26 (most recent played) shows with "Already Played" pill + strikethrough, "I'm watching" toggle works, TopBar swaps to "Watching now · HOPPER" mode. +1 regression test in transform.test.ts covering FRC's null-score shape.

  **Operational note:** during this fix cycle, an inadvertent `cat >> .env.local` (without leading newline check) corrupted `VITE_FRC_TOKEN` by appending `VITE_TBA_API_KEY=...` directly to the token value, causing 401s for ~10 minutes. Restored via `Write` tool. Future env edits should use `Write` (which respects newlines) rather than shell heredoc append.

- **Worlds-week polish (2026-04-30, mid-event)** ✓ — Five UX papercuts shipped together while live at Houston Champs:
  1. **Status fix:** `deriveStatus` now treats alliance slots with `captain ∈ {0, null}` as "selection not held yet" so teams with no schedule no longer flash as `not_selected`. New `alliancesPopulated()` helper in `src/logic/status.ts`. +1 test (17 status tests).
  2. **Results sort earliest-first** — `deriveProgress` now sorts qual + playoff results ascending by match number (was descending). 1 test expectation flipped.
  3. **Schedule fall-off (per favorite)** — `Timeline.tsx` adds a `latestPlayedByFavorite` memo; played matches are visible only if they're the most-recent completed match for at least one of their `myFavorites`. `MatchCard.tsx` derives `played` and renders an "Already Played" pill (`bg-tie/20 text-tie border-tie/40`), strikethrough team rows, and `opacity-70` dim (suppressed under super-match gold ring).
  4. **Collapsible "My Favorites" section** — chevron-toggle button in App.tsx; persists via new key `favorites-section-open/v1`.
  5. **Light mode** — Tailwind `darkMode: 'class'` + new `useTheme` hook (`src/state/theme.ts`, key `theme/v1`, default dark). Pre-paint script in `index.html` avoids light-flash. Sun/moon (`☀ ↔ ☾`) toggle in the header right of the clock. ~115 mechanical migrations of `bg-/text-/border-/divide-neutral-N` classes to paired `light dark:dark` Tailwind variants across 12 components; brand tokens (`purple/gold/feasible/loss/tight/tie/alliance-*`) unchanged.
  6. **Add-favorite race fix** — `useFavorites.add()` no longer relied on `let added` mutated inside an async `setFavorites` updater (the modal saw `added=false` even on first add and showed "Team already added"). Now uses a synchronous `useRef` mirror of state for the dedup check, returning `true` immediately. Existing dedup test still covers the contract.
  All five smoke-tested live in chrome-devtools-mcp (screenshots in `tests/reports/smoke-0[1-3]-*.png`). **102 tests total**.
